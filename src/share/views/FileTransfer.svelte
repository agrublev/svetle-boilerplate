    ...
    function fileTransfer(file) {
        ...
        return new Promise((resolve, reject) => {
            function stream() {
                ...
                if (
                    transferStatus.peers.length === usersCount - 1 &&
                    percentage < transferStatus.percent
                ) {
                    /**
    ...
    onMount(() => {
        ...
        socket.on("userJoin", users => {
            ...
            users.forEach(user => {
                 ...
                 */
                if (backend !== "Using WebRTC") backend = "Using WebSockets";
            });
...
<div id="app" style="text-align:center">
    <header>
        ...
        <h1 class="room-name">{client.room}</h1>

        <!-- Fake element to correct the flex spacing -->
        <button class="thin icon right" style="visibility: hidden"></button>
    ...
    <main class="row file-transfer">
        ...
        <div class="column">
            <canvas bind:this="{canvas}" style="margin-left: -1rem"></canvas>

            {#if percentage !== null}
                <div class="transfer-percentage">{Math.floor(percentage)}%</div>
            {/if}

            <div class="transfer-tech">{backend}</div>
        </div>

        <input id="inpFiles" type="file" hidden on:change="{selectFiles}" multiple />
                        ...
                        {#each files as file}
                            <li>
                                ...
                                {#if file.sent}
                                    ...
                                    <div class="file-status icon-checkmark"></div>
                                {:else}
                                    ...
                                    <svg width="50" height="50" class="file-status">
                                        <circle
                                            ...
                                            cy="25"
                                            r="10"
                                            style="{`stroke-dashoffset:${(63 * percentage) / 100 - 63}`}"
                                        ></circle>
                                    </svg>
                                {/if}
                            </li>
